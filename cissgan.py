...
def load_ci_ssgan_model(model_variant='100p', device=None):
    ...
    print(f"🔧 Loading Ci-SSGAN {model_variant} model...")
...
def prepare_input_data(df):
    ...
    if 'MRN' not in df.columns:
        raise ValueError("❌ 'MRN' column is required")

    if 'input_txt' not in df.columns:
        raise ValueError("❌ 'input_txt' column is required")
    ...
    if has_demographics:
        print("✅ Demographics found (age, race, gender) - using for improved predictions")
        ...
        print(f"⚠️  Demographics not found ({', '.join(missing)}) - using default values")
...
print("\n1️⃣ Basic inference (100p model):")
print("   results = run_inference(test_data)")

print("\n2️⃣ Use 25p model for different clinical domains:")
